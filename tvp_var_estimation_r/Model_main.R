...
start_t <- 1 # Set 2016M1 as a start date
end_t <- 73
...
y <- c("", "Index, 2016 = 100", "%, YoY", "%", "Index, 2016 = 100", "Index", "Index", "Index")
plot_names <- c("", "Industrial Production Index", "CPI", "Monetary policy rate ",
                "NEER", "Terms of Trade", "EXPI", "IMPI")

for (i in 1:length(df_list)) {
  plots[[i]] <- list()
  for(j in 2:length(df_list[[i]])) {
    plots[[i]][[j]] <- df_list[[i]] %>% dplyr::select(1, j) %>% autoplot() + theme_minimal() +
      labs(title = plot_names[j], x = NULL, y = y[j])
  }
  plots[[i]] <- compact(plots[[i]])
}

grid.arrange(grobs = c(plots[[1]]), nrow = 4, ncol = 2)
grid.arrange(grobs = c(plots[[2]][c(1:5)]), nrow = 3, ncol = 2)
grid.arrange(grobs = c(plots[[3]]), nrow = 4, ncol = 2)
grid.arrange(grobs = c(plots[[4]]), nrow = 4, ncol = 2)
...
k_S <- 0.01 # The covariances
k_W <- 0.1 # The volatilities
...
#saveRDS(fit1, "fit_tvp_base.Rds")

# Load model
#fit1 <- readRDS("fit_tvp_base.Rds")
...
# Save the list with estimation results to reduce code running time later
#saveRDS(irf_list, "irf_list_base.Rds")
#saveRDS(irf_x_to_y_median, "irf_x_to_y_median_base.Rds")
#saveRDS(irf_x_to_y_16, "irf_x_to_y_16_base.Rds")
#saveRDS(irf_x_to_y_84, "irf_x_to_y_84_base.Rds")

# Load models
#irf_list <- readRDS("irf_list_base.Rds")
#irf_x_to_y_median <- readRDS("irf_x_to_y_median_base.Rds")
#irf_x_to_y_16 <- readRDS("irf_x_to_y_16_base.Rds")
#irf_x_to_y_84 <- readRDS("irf_x_to_y_84_base.Rds")

## 1) Starting from different time points
# Create the vector of shock names
shock_names <- c("GDP", "CPI", "Interest Rate", "NEER", "ToT")
...
p1 <- grid.arrange(grobs = c(irf_plots[[1]][2:5]), nrow = 2, ncol = 2) # GDP shocks
p2 <- grid.arrange(grobs = c(irf_plots[[2]][c(1, 3:5)]), nrow = 2, ncol = 2) # CPI shocks
p3 <- grid.arrange(grobs = c(irf_plots[[3]][c(1:2, 4:5)]), nrow = 2, ncol = 2) # IR shocks
p4 <- grid.arrange(grobs = c(irf_plots[[4]][c(1:3, 5)]), nrow = 2, ncol = 2) # NEER shocks
p5 <- grid.arrange(grobs = c(irf_plots[[5]][1:4]), nrow = 2, ncol = 2) # ToT shocks

# Save plots
ggsave("irf_gdp.jpeg", plot = p1, units = "px", width = width, height = width/1.61803, device = 'jpeg', dpi = 300)
ggsave("irf_cpi.jpeg", plot = p2, units = "px", width = width, height = width/1.61803, device = 'jpeg', dpi = 300)
ggsave("irf_ir.jpeg", plot = p3, units = "px", width = width, height = width/1.61803, device = 'jpeg', dpi = 300)
ggsave("irf_neer.jpeg", plot = p4, units = "px", width = width, height = width/1.61803, device = 'jpeg', dpi = 300)
ggsave("irf_tot.jpeg", plot = p5, units = "px", width = width, height = width/1.61803, device = 'jpeg', dpi = 300)
ggsave("irf_gdp_cpi.jpeg", plot = irf_plots[[1]][[2]], units = "px", width = width, height = width/1.61803, device = 'jpeg', dpi = 300)

## 2) Responses to a selected shock with 68% confidence interval
# Select shock
shock <- 1 # GDP shock
#shock <- 3 # IR shock
#shock <- 4 # NEER shock
...
for (i in 1:length(irf_x_to_y_median[[3]])) {
  ...
  for(j in 1:length(irf_x_to_y_median[[3]][[1]])) {
    irf_plots_ci[[i]][[j]] <- ggplot(data.frame("Time" = seq(1:37),
                                                "median" = irf_x_to_y_median[[shock]][[i]][[j]],
                                                "p_16" = irf_x_to_y_16[[shock]][[i]][[j]],
                                                "p_84" = irf_x_to_y_84[[shock]][[i]][[j]]),
                                     aes(x = Time, y = median)) + 
      geom_line(col = 'red') + 
...
## 3) Responses to a selected shock with 68% confidence interval at the n-th month  of shock
irf_fit_nth <- list()
...
for (h in 1:length(nth)){
  ...
  for (i in 1:length(irf_x_to_y_median[[3]])){
    ...
    for (k in 1:(n-lag_var)) {
      temp <- data.frame("median" = irf_x_to_y_median[[shock]][[i]][[k]],
                         "p_16" = irf_x_to_y_16[[shock]][[i]][[k]],
                         "p_84" = irf_x_to_y_84[[shock]][[i]][[k]])
...
for (i in 1:length(irf_fit_nth)) {
  ...
  for(j in 1:length(irf_fit_nth[[1]])) {
    irf_plots_nth[[i]][[j]] <- ggplot(irf_fit_nth[[i]][[j]], aes(x = Date, y = median)) +
      ...
      geom_ribbon(aes(ymin = p_16, ymax = p_84), alpha = 0.1) +
      labs(title = paste0("Response of ", shock_names[j], " at ",
...
nth_point <- 4 # Responses at the 12-th month
p6 <- grid.arrange(irf_plots_nth[[nth_point]][[1]],
                   irf_plots_nth[[nth_point]][[2]],
                   irf_plots_nth[[nth_point]][[3]],
                   irf_plots_nth[[nth_point]][[5]],
                   nrow = 2, ncol = 2)

# Save plots
ggsave("irf_ir_6.jpeg", plot = p6, units = "px", width = width, height = width/1.61803, device = 'jpeg', dpi = 300)
ggsave("irf_gdp_24.jpeg", plot = irf_plots_nth[[nth_point]][[2]], units = "px", width = width, height = width/1.61803, device = 'jpeg', dpi = 300)
