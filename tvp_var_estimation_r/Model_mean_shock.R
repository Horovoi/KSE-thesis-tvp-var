# Complementary model with shocks fixed at the sample mean
...
# save the list with estimation results to reduce code running time later
#saveRDS(irf_list, "irf_list_ms.Rds")
#saveRDS(irf_x_to_y_median, "irf_x_to_y_median_ms.Rds")
#saveRDS(irf_x_to_y_16, "irf_x_to_y_16_ms.Rds")
#saveRDS(irf_x_to_y_84, "irf_x_to_y_84_ms.Rds")

# load model
#irf_list <- readRDS("irf_list_ms.Rds")
#irf_x_to_y_median <- readRDS("irf_x_to_y_median_ms.Rds")
#irf_x_to_y_16 <- readRDS("irf_x_to_y_16_ms.Rds")
#irf_x_to_y_84 <- readRDS("irf_x_to_y_84_ms.Rds")
...
p_ms_1 <- grid.arrange(grobs = c(irf_plots[[3]][c(1:2, 4:5)]), nrow = 2, ncol = 2) # IR shocks
grid.arrange(grobs = c(irf_plots[[4]][c(1:3, 5)]), nrow = 2, ncol = 2) # NEER shocks
grid.arrange(grobs = c(irf_plots[[5]][1:4]), nrow = 2, ncol = 2) # ToT shocks

# save plots
ggsave("irf_ms_ir.jpeg", plot = p_ms_1, units = "px", width = width, height = width/1.61803, device = 'jpeg', dpi = 300)
