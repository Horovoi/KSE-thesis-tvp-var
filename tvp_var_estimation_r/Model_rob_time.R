# Complementary model on data sample from 2016M5 to 2022M1
...
start_t <- 5 # Set 2016M5 as a start date
end_t <- 73
...
k_S <- 0.01 # The covariances — PRIMICERY GOT 0.1
k_W <- 0.1 # The volatilities — PRIMICERY GOT 0.01
...
#saveRDS(fit1, "fit_tvp_rt.Rds")

# load model
#fit1 <- readRDS("fit_tvp_rt.Rds")
...
# save the list with estimation results to reduce code running time later
#saveRDS(irf_list, "irf_list_rt.Rds")
#saveRDS(irf_x_to_y_median, "irf_x_to_y_median_rt.Rds")
#saveRDS(irf_x_to_y_16, "irf_x_to_y_16_rt.Rds")
#saveRDS(irf_x_to_y_84, "irf_x_to_y_84_rt.Rds")

# load model
#irf_list <- readRDS("irf_list_rt.Rds")
#irf_x_to_y_median <- readRDS("irf_x_to_y_median_rt.Rds")
#irf_x_to_y_16 <- readRDS("irf_x_to_y_16_rt.Rds")
#irf_x_to_y_84 <- readRDS("irf_x_to_y_84_rt.Rds")
...
for (i in 1:length(irf_x_to_y_median)) {
  ...
  for(j in 1:length(irf_x_to_y_median)) {
    irf_plots[[i]][[j]] <- autoplot(ts(irf_x_to_y_median[[i]][[j]][[1]]), series = "2016 Jul") +
      autolayer(ts(irf_x_to_y_median[[i]][[j]][[45]]), series = "2020 Apr") +
      autolayer(ts(irf_x_to_y_median[[i]][[j]][[66]]), series = "2022 Jan") +
...
# Plot IRF starting from different time points (2016 Jul, 2020 Apr, 2022 Jan)
...
p_rt_1 <- grid.arrange(grobs = c(irf_plots[[3]][c(1:2, 4:5)]), nrow = 2, ncol = 2) # IR shocks
p_rt_2 <- grid.arrange(grobs = c(irf_plots[[4]][c(1:3, 5)]), nrow = 2, ncol = 2) # NEER shocks
grid.arrange(grobs = c(irf_plots[[5]][1:4]), nrow = 2, ncol = 2) # ToT shocks

# save plots
ggsave("irf_rt_ir.jpeg", plot = p_rt_1, units = "px", width = width, height = width/1.61803, device = 'jpeg', dpi = 300)
ggsave("irf_rt_neer.jpeg", plot = p_rt_2, units = "px", width = width, height = width/1.61803, device = 'jpeg', dpi = 300)

ggsave("irf_rt_gdp_cpi.jpeg", plot = irf_plots[[1]][[2]], units = "px", width = width, height = width/1.61803, device = 'jpeg', dpi = 300)
...
axy <- list(ticktext = y_names[c(1, seq(10, 66, 12))], tickvals = c(1, (seq(10, 66, 12))),
            range = c(0, 66), title = "Date of the shock")
...
# Build tables with MCMC draws at 2016 Jul, 2020 Apr and 2022 Jan
...
for (h in 1:length(nth)){
  ...
  for (i in 1:length(irf_x_to_y_median[[shock_prob]])){
    irf_mcmc_nth[[h]][[i]] <- data.frame(irf_list[[shock_prob]][[i]][[1]][,nth[h]],
                                         irf_list[[shock_prob]][[i]][[45]][,nth[h]],
                                         irf_list[[shock_prob]][[i]][[66]][,nth[h]])
    colnames(irf_mcmc_nth[[h]][[i]]) <- c(y_names[1], y_names[45],y_names[66])
  }
}
